<script setup lang="ts">
    ...
    import type { Visitante, Sala, Usuario } from '../../types';
    ...
    const usuario: Usuario | undefined = store.usuario;

    const nomeCookie = useCookie('usuario');
    const visitante = ref<Visitante | null>(null);
    const nome = ref<string>('');

    const revelado = ref<boolean>(false);

    const criaVisitante = async () => {
        let nomeVisitante: string = nomeCookie.value ? nomeCookie.value : '';

        if (usuario)
            nomeVisitante = usuario.nome;

        visitante.value = { nome: nomeVisitante, nota: 0, sala: salaPrm };
        
        entrar(nomeVisitante);
    }

    const entrar = async (nome: string) => {

        let usrTmp: Visitante = visitante.value ? visitante.value : { nome: nome, nota: 0, sala: salaPrm };

        try {
            let resposta: Visitante = await store.addVisitante(usrTmp);
            if (resposta?.nome) {
                nomeCookie.value = resposta.nome;
            }
            visitante.value = resposta;
            fetchVisitantes();
        } catch (err) {
            console.error(err);
        }
    ...
    const sair = () => {
        if (visitante.value) {
            store.removerVisitante( visitante.value );
        }
        nomeCookie.value = null;
        nome.value = '';
    }

    const mudaNota = (nota: number) => {
        let usrTmp: Visitante = { nome: nomeCookie.value as string, nota: nota, sala: salaPrm };
